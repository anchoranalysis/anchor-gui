...
public class Usher implements ContainerListener {
    ...
    /**
     * Recursively hop from component to component and try to find free space next to it.
     *
     * @param visited a boolean array that serves as a safeguard against going into infinite
     *     recursions when frames overlap.
     ...
     * @return Either the point to put the target on or null if no free space can be found.
     */
    private Point descend(
            boolean[][] visited, Dimension size, Component[] all, Component target, Point test) {
                ...
                if (!visited[i][0]) {
                    ...
                    ne =
                            descend(
                                    ...
                                    all,
                                    target,
                                    new Point(tmp.x + tmp.width + 1 + PADDING, tmp.y));
                }
                if (!visited[i][1]) {
                    ...
                    sw =
                            descend(
                                    ...
                                    all,
                                    target,
                                    new Point(tmp.x, tmp.y + tmp.height + 1 + PADDING));
                }
    ...
    /**
     * Hop to equispaced anchor points along a diagonal line, trying to find a free one.
     *
     * @param start region bounds
     * @param size region bounds
     * @param all all components in the container
     * @return a free point to place the component on or null if none is found within the bounds.
     */
